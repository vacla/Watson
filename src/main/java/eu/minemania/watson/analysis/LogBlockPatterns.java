...
{
    public static final Pattern LB_POSITION 			= Pattern.compile("^Block changes in the last \\d+ \\w+ at (-?\\d+):(-?\\d+):(-?\\d+) in (.+):$");
    public static final Pattern LB_KILLS	 			= Pattern.compile("^Kills in the last \\d+ \\w+ in (.+):$");
    public static final Pattern LB_COORD_POSITION		= Pattern.compile("^Block changes in the last \\d+ \\w+ in (.+):$");
    public static final Pattern LB_EDIT 				= Pattern.compile("^((?:\\d{2,4}-)?\\d{2}-\\d{2}) (\\d{2}):(\\d{2}):(\\d{2}) (\\w+) (created|destroyed) ((?: |\\w)+)( \\[.*\\] \\[.*\\] \\[.*\\] \\[.*\\])?$");
    public static final Pattern LB_EDIT_REPLACED 		= Pattern.compile("^((?:\\d{2,4}-)?\\d{2}-\\d{2}) (\\d{2}):(\\d{2}):(\\d{2}) (\\w+) replaced ((?: |\\w)+) with ((?: |\\w)+)$");
    public static final Pattern LB_COORD 				= Pattern.compile("^\\((\\d+)\\) ((?:\\d{2,4}-)?\\d{2}-\\d{2}) (\\d{2}):(\\d{2}):(\\d{2}) (\\w+) (created|destroyed) ([a-zA-Z_]+)(?: \\[(?<sign1>.*)\\] \\[(?<sign2>.*)\\] \\[(?<sign3>.*)\\] \\[(?<sign4>.*)\\])? at (-?\\d+):(\\d+):(-?\\d+)$");
    public static final Pattern LB_COORD_KILLS          = Pattern.compile("^\\((\\d+)\\) ((?:\\\\d{2,4}-)?\\d{2}-\\d{2}) (\\d{2}):(\\d{2}):(\\d{2}) (\\w+) killed (\\w+) at (-?\\d+):(\\d+):(-?\\d+) with (.*)$");
    public static final Pattern LB_COORD_REPLACED       = Pattern.compile("^\\((\\d+)\\) ((?:\\d{2,4}-)?\\d{2}-\\d{2}) (\\d{2}):(\\d{2}):(\\d{2}) (\\w+) replaced ([a-zA-Z_]+) with ([a-zA-Z_]+) at (-?\\d+):(\\d+):(-?\\d+)$");
    ...
    public static final Pattern LB_HEADER_TIME_CHECK    = Pattern.compile("Block changes from player \\w+ between (\\d+) and \\d+ minutes ago in .+:");
